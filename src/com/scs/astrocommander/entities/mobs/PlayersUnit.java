...
public class PlayersUnit extends AbstractMob {
	...
	public void process() {
		...
		if (this != this.main.getCurrentUnit()) {
			checkForShooting();
		}
		...
		if (status == Status.Waiting) {
			if (this.food < 99f) {
				MapSquareReplicator sqr = (MapSquareReplicator)m.gameData.map_data.findAdjacentMapSquare(x, y, AbstractMapSquare.MAP_REPLICATOR);
				if (sqr != null) {
					if (m.gameData.totalPower > 0) {
						m.gameData.powerUsedPerTurn += 1f;
						this.incFood(10);
		...
		// Are we on a medibay?
		if (status == Status.Waiting) {
			AbstractMapSquare sq = (AbstractMapSquare)this.getSq();
			if (sq.type == AbstractMapSquare.MAP_MEDIBAY) {
				if (m.gameData.totalPower > 0 && this.health < 100) {
					m.gameData.powerUsedPerTurn += 1f;
					this.incHealth(1, "");
		...
		// Repair something
		if (status == Status.Waiting) {
			AbstractMapSquare sqr = m.gameData.map_data.findAdjacentRepairableMapSquare(x, y);
			if (sqr != null) {
		...
		// Moving
		if (status == Status.Waiting && this != main.getCurrentUnit()) {
		...
		//} else if (pass == 2) {
		m.gameData.incOxygenLevel(-0.1f);
		if (m.gameData.oxygenLevel <= 0) {
	...
	public void useConsole() {
		Main m = (Main)this.main;
		MapSquareControlPanel sq = (MapSquareControlPanel)m.gameData.map_data.findAdjacentMapSquare(x, y, AbstractMapSquare.MAP_CONTROL_PANEL);
		if (sq != null) {
			main.sfx.playSound("scanner.mp3");
			main.setModule(new CommandConsole((Main)main, main.getCurrentModule()));
		} else {
